...
H0: Restoration method has no effect on restoration success

HA: The popsicle stick restoration method is more successful than the garden staple
...
total <- nrow(restoration)
print(total) 

observed_stat <- restoration %>% 
  group_by(treatment) %>% 
  summarise(proportion = sum(success) / n())

diff_in_prop <- diff(observed_stat$proportion)
```
...
null_dist_df <- tibble(null_distribution) 

ggplot(null_dist_df, aes(x = null_distribution)) +
  geom_histogram(binwidth = 0.05) +
  geom_vline(xintercept = diff_in_prop, col = 'firebrick', lwd = 1.5) +
  theme_minimal()
...
p_val <- mean(null_distribution > diff_in_prop)
...
The second interpretation is correct, since we did not find evidence to support the null hypothesis
...
bootstrap <- function(i) {
  one_bootstrap <- restoration %>% 
    group_by(treatment) %>% # shuffle within treatment
    mutate(success = sample(success,
                            size = length(success),
                            replace = TRUE)) %>%
    ungroup() %>% 
    mutate(trial = i) # Why do we do this?
}
...
boot_diff_prop <- bootstrap_samples %>% 
  ...
  summarise(prop_success = mean(success),
            .groups = "drop_last") %>% 
  summarise(diff_prop = diff(prop_success))
```
...
diff_prop <- boot_diff_prop$diff_prop
...
ggplot(boot_diff_prop, aes(diff_prop)) +
  geom_histogram() +
  geom_vline(xintercept = diff_in_prop, 
             color = "firebrick",
             linewidth = 1.5) +
...
Break the association between treatment and outcome by shuffling treatment without replacement

**Q19: What did you do to make the bootstrap distribution center on the observed test statistic? Specifically, what code?**

Retain the association between treatment and outcome. Grouped by treatment, then sampled with replacement.

**Q20: What would happen to your bootstrap distribution if you sampled without replacement?**

Sampling without replacement is just shuffling and since we sampled within treatments, the bootstrap samples would always match the observed (no variation in the bootstrap distribution of the test statistic).
