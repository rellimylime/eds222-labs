...
At zero, the gage should be at the value $\beta_0$\
For every inch of rain the gage should go up by $\beta_1$
...
# Compute y hat using our beta coefficients and extracted x values
y_hat <- 3.9 + 2.2 * x
...
$y$ is the actual observed values of y - observed gage heights

$\bar y$ is the mean of all the y's - mean gage height (overall)

$\hat y$ expected value of y given x - expected gage height given precipitation

**Q5: What does the** $R^2$ **value tell you about the quality of the model (in plain language)?**

Having precipitation reduces the error of our predictions by \~55%
...
rreas_murres <- read_csv("week2/data/rreas.csv")
...
ggplot(rreas_murres, aes(rreas_rockfish_index, comu_rockfish_diet)) +
  geom_point()
